...
A branch of the trace is dropped as soon as its counter exceeds ``order`` or its power falls below ``power_threshold``, whichever comes first: the one bounds how *deep* a ghost may be, the other how *faint*. ``order`` is a budget for the ghosts a call may **make**, not a test the arriving beam has to pass — a reflection off a face meant to reflect makes none, so an already-stray beam bounces off a mirror however deep it has got, and leaves at the order it arrived at.

**The counter rides with the beam.** It is not reset when a beam leaves one element for the next, so ``order`` is a budget for the whole trace: a ghost made at one mirror and steered by another is still that ghost, drawn as stray and carrying its count with it. That is also what bounds the recursion — each branch ends when it has spent the budget — so a system of mirrors facing each other terminates on ``order`` rather than waiting for the power to run out.
