...
Placing a follower is refused, in the same words a held body already uses: it goes where its host goes, so ``move``, ``align``, ``slide`` and a typed pose are turned away. A pose written into one would be overwritten at the next trace, which is worse than a refusal because it would look as though it had worked. The exception is the turn of an element whose ``fix_rotation`` is False, which is read back into the joint so that the next settle keeps it. :py:meth:`copy_optics<gtrace.layout.OpticalLayout.copy_optics>` brings the followers along — the second face of a dump is part of the dump.

**Removing something takes what stands on it.** :py:meth:`remove_optics<gtrace.layout.OpticalLayout.remove_optics>` and :py:meth:`remove_mechanics<gtrace.layout.OpticalLayout.remove_mechanics>` take the target and everything whose pose comes from it, however deep, and return the list of what went. A mount whose mirror has gone would derive its pose from a ghost, and asking for it to be taken away separately would be asking twice for one thing. It is one edit, so it is one step of undo. Detach or disassemble first to keep something that is standing on it — that is what says "this one is its own now".
