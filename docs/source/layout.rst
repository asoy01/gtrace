...
that govern the non-sequential trace. You draw the layout, save it to disk,
and edit it in the interactive viewer.
...
:doc:`propagation` also work on their own, and the :doc:`tutorial` traces a
mode cleaner without a layout.

Build the system, then register it
-----------------------------------

The work has two phases. First, build and align the system with ordinary
Python:
...
Then register the optics and the source in one layout:
...
cavity eigenmode solving. These methods close a cavity on itself, or make a
beam arrive at a mirror at the intended angle of incidence. The layout
...
returns the same object that ``PRM`` names in your code. So:
...
that ``PRM`` names. There is one copy of the model, and your code and the
viewer both use it.
...
:py:class:`TraceRules<gtrace.layout.TraceRules>` collects the settings that
govern the non-sequential trace:

``order``
    How many ghost reflections a beam may make before the trace stops
    following it. Defaults to 10.

``power_threshold``
    Beams weaker than this threshold are not propagated further. Defaults
    to 0.1. Change this value when you look for ghost beams. A lower value
    makes the trace find fainter paths, and take longer.
...
many ghosts of *one particular element* are followed is a property of that
element, the ``max_stray_order`` attribute of the optics.
:ref:`stray-order` describes what raises a beam's stray order, and the
flags that mark the role of each face of an element.

``open_beam_length`` applies to the beams the trace produces. A *source*
that reaches nothing keeps its own ``length``. That happens often while a
layout is still being built, before any element stands in the beam's path.
...
an option redraws the layout; it does not re-trace. The available options
and their defaults are collected in ``gtrace.layout.DRAW_OPTIONS``:
...
``drawMechanicsNames``       False      Annotate each body with its name
...
every envelope at the same σ gives them all the same meaning: outside the
line there is nothing that matters at the ppm level.
...
:py:meth:`show<gtrace.layout.OpticalLayout.show>` works immediately after a
load.
...
between two points of the layout, and is stored in the layout:

.. code-block:: python

    from gtrace.layout import Dimension

    layout.add_dimension(Dimension(PRM.HRcenter, PRM.ARcenter,
                                   name='D1', offset=0.17))
    layout.get_dimension('D1').length          # in metres

Dimensions are registered on the layout beside the optics, saved with it,
and restored by undo. They share one namespace with the optics.

``offset`` is where the dimension *line* is drawn, in metres, positive to
the left of the direction from ``p1`` to ``p2``.
:py:meth:`line_ends<gtrace.layout.Dimension.line_ends>` gives the two ends
of that line. The offset is a choice about the drawing, not about the
measurement. The distances you want to measure usually run along a beam or
through an element, where a line drawn on top of them cannot be read. **The
offset changes nothing about what is measured.** The span, the length and
the test below are all still taken between ``p1`` and ``p2``.

**A dimension does not remember the element a point was taken from.** An
element that then moves leaves the measurement where it was made. To measure
the same distance after a change, add the dimension again.

The value of a dimension is computed every time the scene is built, and
never stored, so it cannot become out of date:
...
``optical`` is how you measure the optical thickness of a substrate. A span
...
so gtrace does not report it.

:py:meth:`contains_segment<gtrace.optcomp.Optics.contains_segment>` answers
whether a span runs inside one substrate, by asking the optics itself.
...
and moves and turns with the host. ``M1`` and ``M2`` below are two steering
mirrors registered in the layout:

.. code-block:: python

    layout.assemble('M2', 'M1')                  # keeps where it stands
    layout.assemble('M2', 'M1', offset=[0.06, 0.0],
                    offset_angle=deg2rad(-40))   # or at a place you name
    layout.disassemble('M2')

The offset is in the host's frame: origin at its substrate centre, x along
its HR normal, the same frame a mount attaches in. The point that lands
there is the follower's **anchor point**, the point it is already held by.
``fix_rotation`` decides who may change the relative angle. True, the
default, suits a face of a dump, which is built at its angle. False lets the
angle be edited, and the element still turns *with* its host. The host may
be another element or a body.
...
joint, so that the angle is kept the next time the follower's pose is
written from the host.
:py:meth:`copy_optics<gtrace.layout.OpticalLayout.copy_optics>` copies the
followers too, because the second face of a dump is part of that dump.
...
One consequence follows. Reading a follower's pose in a cell **without**
...
**Removing an object takes what stands on it.**
...
return the list of what was removed. A mount whose mirror has gone would
derive its pose from an object no longer in the layout. The removal is one
edit, so it is one step of undo. To keep a body that stands on the target,
detach or disassemble it first.

A cycle is refused, in a call and in a loaded file: a pose that comes from
itself has no value.

The beam dump
^^^^^^^^^^^^^^

:py:meth:`add_beam_dump<gtrace.layout.OpticalLayout.add_beam_dump>` builds a
beam dump and registers it: two absorbing faces in a V and the housing they
sit in, jointed so the three move as one.
...
the beam runs, not by where its mouth points. The call returns ``(optics,
bodies)``, like every builder here. Each list has the hosts first, the order
they are registered in: ``BD1a`` is the host, ``BD1b`` follows it and
``BD1box`` stands on it. The pieces are named after the dump: ``BD1a`` and
``BD1b`` are its two faces and ``BD1box`` its housing. A dump is numbered
and its pieces lettered, so ``BD2b`` is the far face of the second dump. A
dump with no name given takes the first free number.
:py:func:`beam_dump<gtrace.layout.beam_dump>` builds the same three pieces
without registering them.

The two faces stand in a V because a black face is not perfectly black. One
face reflects a little light, the other catches it and reflects it again, so
the beam travels deeper into the wedge instead of leaving the way it came.
With the default 4% a beam falls to 0.16% after two bounces and to a part
...
The reflectivity of a black absorber depends on the glass and the
polarisation, so
:py:data:`DUMP_REFLECTIVITY<gtrace.layout.DUMP_REFLECTIVITY>` is a
placeholder. Measure it and pass your own value.

Aim the beam into one side of the V, not at the apex. A ray sent exactly at
the apex hits neither face, because the apex is where both faces end. The
same is true of a real dump.
...
A bench is not only beams. The hardware that holds the optics takes up
room, collides with other hardware and has to be bolted somewhere, so a
layout carries it too. A :py:class:`Mechanics<gtrace.mechanics.Mechanics>`
is a named body of drawing primitives that the trace never sees:
...
:py:meth:`world_shapes<gtrace.mechanics.Mechanics.world_shapes>` gives the
shapes in world coordinates. It builds new primitives, and does not move the
ones you hold.

A body is drawn, picked, measured, saved and exported like the optics, and
it is invisible to the beams: adding, moving or editing one does not
invalidate a trace. A body goes on the ``mechanics`` layer by default, so a
DXF or the viewer's layer panel can hide every body at once.
:py:meth:`contains<gtrace.mechanics.Mechanics.contains>` decides what a
click lands on, with a point-in-polygon test against the body's
:py:meth:`outline<gtrace.mechanics.Mechanics.outline>`. Of several bodies
under one point the smallest is picked, so a mount standing on a breadboard
is not shadowed by the breadboard.
...
copy that could become out of date. Move the mirror in a cell, in the
viewer, or by loading a file, and the mount is already where it should be.
The cost is that an attached body cannot be moved on its own. Writing to its
pose, or dragging it, is refused.

Where a body sits on its host is a **coordinate convention**, not a number.
The local origin of a part is the point that lands at the centre of the
host's substrate. A mount is therefore drawn around the mirror it holds, and
needs no offset to land correctly. Use ``offset`` and ``offset_angle`` to
place a body slightly off its host.

**A body may stand on another body.** Parts stack on a bench: the mount is
bolted to a pedestal, the pedestal is held down by a clamping fork.
``attached_to`` takes either one. The chain follows the optics at its root,
and a cycle is refused.
:py:func:`host_pose<gtrace.mechanics.host_pose>` is the one place that knows
the difference between the two kinds of host: an optics is turned by its HR
normal, and a body by its own angle.

When one body stands on another, what differs is where the body is pinned.
A mount is pinned by its origin, the point drawn to coincide with its
...
in its own coordinates, and the pose derives from that point:
...
the rule matches how every mount is already drawn.
...
All of this is built on **named points**. ``points`` is a dict of local
points a part names for itself: ``'post'`` for the hole under a mount,
``'axis'`` for a pedestal, ``'bore'`` and ``'screw'`` for a fork. The dict
travels with the model in the library. Named points join the snap points, so
a drag settles on them, a measurement reaches them, and Align aims by them.

:py:meth:`detach<gtrace.mechanics.Mechanics.detach>` stores the derived pose
and frees the body. :py:meth:`attach<gtrace.mechanics.Mechanics.attach>`
...
:py:meth:`copy_optics<gtrace.layout.OpticalLayout.copy_optics>` copies an
element **with the whole stack standing on it**: the mount bolted to it, the
pedestal under the mount, and the fork over the pedestal. Each part is
pinned to the copy the way its original is pinned to the original. One of a
pair of steering mirrors is the element plus everything built under it, and
you do not want to build that twice.
...
first free number after it, so a copy of ``M1`` is ``M2``. Without an
offset the copy stands its own diameter away along both axes, far enough to
clear the original.
...
A bench does not carry a bare mirror. It carries a mirror in a mount, on a
pedestal, held down by a fork.
:py:meth:`add_assembly<gtrace.layout.OpticalLayout.add_assembly>` builds one
and registers it:
...
The call returns four objects: the element first, and then the parts that
hold it. Each part is attached to the one below it. The mount goes on the
optic at the position the model was designed for. The pedestal goes in the
hole the mount is bolted down through. The fork goes round the pedestal,
with its turn free. **Move the element, not the parts.** Everything else
derives its pose from the element.
...
the two each kind is placed by, under ``place``, and the viewer sends that
key when an assembly is placed by a click.

``add_assembly`` fills in the names as it registers the four. Each piece
takes the first number that is free for its own kind, so a second two-inch
mirror is named ``M2``, held by ``MT2`` on ``P2`` in ``FK2``.
:py:func:`assembly<gtrace.layout.assembly>` builds the same four without
registering them.

The kinds are built from
:py:func:`mirror_assembly<gtrace.layout.mirror_assembly>` and
:py:func:`lens_assembly<gtrace.layout.lens_assembly>`. They take the same
arguments, plus the models to build the parts from. Pass ``None`` for a
model to leave that piece out.
...
is a bench measurement, and the model does not know it. The hole the
pedestal stands in moves with the mount, because the post hole is a point of
the mount.
...
A model is a **value**: the shapes, a layer, a description, the prefix its
parts are named with, and the builder parameters. gtrace copies them in when
the model is registered, and copies them out when it is used, so a body and
the model it came from stay independent.

``prefix`` is the name the bodies built from the model take, before the
...
gives no prefix leaves the naming to whoever adds the body; the viewer uses
``H``. Loading a file merges the models into the registry name by name, so a
library can be assembled from several files. A file with one bad
...
**The saved layout is the truth**: a later change to the library cannot
change a drawing you already made.
:py:meth:`relink_mechanics<gtrace.layout.OpticalLayout.relink_mechanics>`
loads the newer definition, and changes neither pose nor attachment.
...
size, and ``None`` for a body drawn by hand, where only the shapes are
known. **A round body has one size, not two:** either ``width`` or
...
editor on a body. The editor is the same viewer, with a scene that holds
only the shapes, drawn in the local frame with the origin marked. See
...
state before the edit, so the edit can be undone. An undo keeps the state it
leaves, so the edit can be redone:
...
thing however the edit arrived. The history goes back ``UNDO_DEPTH`` edits.
The redo side has the same limit, because only an undo fills it. A refused
edit changes nothing and costs no step, and neither does ``save``, which
only writes a file.

**An edit that goes through discards whatever is waiting to be redone.**
You cannot return to the states you stepped out of, because they describe
elements that the new edit may have renamed, removed, or changed.
...
This works through a rename, and through a removal, since an element taken
...
A layout is edited by the viewer, and by any other front end that uses the
same protocol, through
...
one may touch, and what the scene returns.
