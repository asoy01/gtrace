...
There is one add button per kind of thing: ``+ Mirror``, ``+ Lens``, ``+ Source``, ``+ Dump`` and ``+ Mechanics``. The two that have variants open on the choice between them, spherical or cylindrical, since a cylindrical mirror is a mirror and not a fifth kind of thing. ``+ Mechanics`` opens on the model library, which is a list. ``+ Dump`` puts a :ref:`beam dump <mechanics>` down facing a beam that runs along +x. A dump is three pieces — two absorbing faces and the housing they sit in — so it is a kind of its own, and it cannot be on the library shelf that ``+ Mechanics`` opens, because a model holds shapes and two thirds of a dump are elements.
...
Every gesture commits as one message, so it is one step of undo and goes through the same constructor a typed row does. **A turned rectangle becomes an outline**, because a ``Rectangle`` has its sides along the axes and no turned form; one undo puts the rectangle back. The editor works on the body itself, by reference, so a part already registered in a layout is redrawn there as soon as the layout is drawn again.
